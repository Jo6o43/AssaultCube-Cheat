...
def getInfo():
    ...
    try:
        ...
        print(f"Players: {num_players} | Lista: {hex(entity_list_ptr)}")
...
def get_view_matrix():
    try:
        ...
        VIEW_MATRIX_ADDR = module + 0x17DFD0
        view_raw = pm.read_bytes(VIEW_MATRIX_ADDR, 64)
        print (f"VIEW_MATRIX_ADDR: {hex(VIEW_MATRIX_ADDR)} | Raw Data: {view_raw.hex()}")
        return struct.unpack('16f', view_raw)
    except Exception as e:
        print(f"\n[!] Erro: {e}")